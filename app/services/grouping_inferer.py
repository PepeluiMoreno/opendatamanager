"""
GroupingInferer — infiere propuestas de agrupación a partir de URLs hoja.

Recibe la lista plana de URLs hoja descubiertas por `WebTreeFetcher.discover()`
y emite `GroupingProposal`s. Cada propuesta colapsa URLs que comparten estructura
modulo dimensiones detectadas (year, month, quarter, date, genérico).
...
Ejemplo:
    ...
    /transparencia/economica/2023/presupuesto.xlsx
        →  path_template = "https://h/transparencia/economica/{year}/presupuesto.xlsx"
           dimensions    = [{"name": "year", "kind": "year", "sample_values": ["2021","2022","2023"]}]
           matched_urls  = [...3...]

Función pura: input = lista de dicts; output = lista de GroupingProposal.
Sin BD, sin red, sin estado.
"""
...
# Segmentos que no aportan información semántica al nombre sugerido.
...
def _template_filename(filename: str, segment_index: int) -> Tuple[str, Optional[Dict[str, Any]]]:
    """Detecta un patrón de año dentro del nombre de fichero (ej. presupuesto_2024.xlsx)."""
...
def infer(leaf_urls: List[Dict[str, Any]]) -> List[GroupingProposal]:
    ...
    for key, entries in grouped.items():
        ...
        # Agregar valores únicos por nombre de dimensión
        dim_values: Dict[str, set] = defaultdict(set)
        for entry_dims in grouped_dims[key]:
            for d in entry_dims:
                dim_values[d["name"]].add(d["value"])

        # Construir lista canónica de dimensiones (orden = primera aparición)
