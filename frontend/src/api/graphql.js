...
export async function fetchDiscoverArtifact(resourceId) {
  try {
    return await client.request(
      `query DiscoverArtifact($resourceId: ID!) { discoverArtifact(resourceId: $resourceId) }`,
      { resourceId }
    )
  } catch (e) { handleGraphQLError(e) }
}
...
export async function createChildResources(parentResourceId, sections) {
  try {
    return await client.request(
      `mutation CreateChildResources($parentResourceId: ID!, $sections: [DiscoverSectionInput!]!) {
        createChildResources(parentResourceId: $parentResourceId, sections: $sections) {
          id name targetTable autoGenerated active
        }
      }`,
      { parentResourceId, sections }
    )
  } catch (e) { handleGraphQLError(e) }
}
