  ...
  GET_RESOURCES: `
    query GetResources($activeOnly: Boolean) {
      resources(activeOnly: $activeOnly) {
        ...
        generaColecciones
        esColeccion
        preset { id code description params }
        fetcher {
          id
          name
          code
...
export const MUTATIONS = {
  ...
  CREATE_RESOURCE_COLLECTION: `
    mutation CreateResourceCollection($name: String!) {
      createResourceCollection(name: $name) { id name origin miembros }
    }
  `,

  RENAME_RESOURCE_COLLECTION: `
    mutation RenameResourceCollection($id: String!, $name: String!) {
      renameResourceCollection(id: $id, name: $name) { id name origin miembros }
    }
  `,

  DELETE_RESOURCE_COLLECTION: `
    mutation DeleteResourceCollection($id: String!) {
...
export async function fetchResourceCollections() {
  try {
    return await client.request(`query { resourceCollections { id name origin miembros rootResourceId parentCollectionId } }`)
  } catch (error) {
    handleGraphQLError(error)
  }
}
...
export async function renameResourceCollection(id, name) {
  try {
    return await client.request(MUTATIONS.RENAME_RESOURCE_COLLECTION, { id, name })
  } catch (error) {
    handleGraphQLError(error)
  }
}

export async function deleteResourceCollection(id) {
  try {
    return await client.request(MUTATIONS.DELETE_RESOURCE_COLLECTION, { id })
  } catch (error) {
    handleGraphQLError(error)
  }
}
...
// ── ResourceCandidate (Discovering / Web Tree) ──────────────────────────────
...
export async function fetchResourceChildren(parentResourceId) {
  try {
    return await client.request(
      `query ResourceChildren($parentResourceId: ID!) {
        resourceChildren(parentResourceId: $parentResourceId) {
          id name targetTable active autoGenerated schedule
          preset { id code description params }
        fetcher { id name code }
          params { id key value isExternal }
        }
      }`,
      { parentResourceId }
    )
...
export async function promoverRama(input) {
  try {
    return await client.request(
      `mutation PromoverRama($input: PromoverRamaInput!) {
        promoverRama(input: $input) {
          id name autoGenerated active parentResourceId
        }
      }`,
      { input }
    )
...
export async function promoteCandidate(id, input) {
  try {
    return await client.request(
      `mutation PromoteCandidate($id: ID!, $input: PromoteCandidateInput!) {
        promoteCandidate(id: $id, input: $input) {
          id name targetTable autoGenerated active parentResourceId schedule
        }
      }`,
      { id, input }
    )
...
export async function discardCandidate(id, reviewer) {
  try {
    return await client.request(
      `mutation DiscardCandidate($id: ID!, $reviewer: String) {
        discardCandidate(id: $id, reviewer: $reviewer) { ${CANDIDATE_FIELDS} }
      }`,
      { id, reviewer: reviewer || null }
    )
  } catch (e) { handleGraphQLError(e) }
}

export async function mergeCandidates(sourceIds, targetId) {
  try {
    return await client.request(
      `mutation MergeCandidates($sourceIds: [ID!]!, $targetId: ID!) {
        mergeCandidates(sourceIds: $sourceIds, targetId: $targetId) { ${CANDIDATE_FIELDS} }
      }`,
      { sourceIds, targetId }
    )
  } catch (e) { handleGraphQLError(e) }
...
export async function splitCandidate(id, groups) {
  try {
    return await client.request(
      `mutation SplitCandidate($id: ID!, $groups: JSON!) {
        splitCandidate(id: $id, groups: $groups) { ${CANDIDATE_FIELDS} }
      }`,
      { id, groups }
    )
  } catch (e) { handleGraphQLError(e) }
