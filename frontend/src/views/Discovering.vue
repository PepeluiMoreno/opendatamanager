<template>
  <div class="p-8 max-w-4xl">
    ...
    <div class="card p-5 mb-6">
      <label class="block text-sm text-gray-400 mb-2">Recurso de descubrimiento</label>
      ...
      <select v-else v-model="selectedResourceId" class="input w-full">
        <option value="">— Selecciona un recurso de descubrimiento —</option>
...
import { fetchResources, executeResource, fetchDiscoverArtifact, fetchDiscoverArtifactForExecution, createChildResources } from '../api/graphql'
...
const discoverableResources = computed(() =>
  allResources.value.filter(r =>
    r.fetcher &&
    !r.autoGenerated &&
    (r.fetcher.classPath?.toLowerCase().includes('legacy_data_portal') ||
     r.fetcher.classPath?.toLowerCase().includes('document_portal') ||
     r.fetcher.code?.toLowerCase().includes('portal') ||
     r.fetcher.name?.toLowerCase().includes('portal'))
  )
)
...
function startPolling(execId) {
  ...
  pollTimer = setInterval(async () => {
    ...
    const r = execId
      ...
      : await fetchDiscoverArtifact(selectedResourceId.value)
    const json = r?.discoverArtifactForExecution ?? r?.discoverArtifact
    if (json) {
      clearInterval(pollTimer)
      loadSections(json)
    }
