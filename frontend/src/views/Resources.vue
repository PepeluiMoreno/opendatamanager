<template>
  <div class="p-6 flex flex-col h-full" ref="viewEl">
        ...
        <tbody>
          ...
          >
            <td class="py-1.5 px-3 text-xs font-medium text-white whitespace-nowrap">
              {{ resource.name }}
              <span v-if="resource.autoGenerated"
                    class="ml-1.5 inline-block text-[9px] uppercase tracking-wide font-bold px-1 py-0.5 rounded bg-purple-900 text-purple-300 border border-purple-700">auto</span>
            </td>
            ...
            <td class="py-1.5 px-3">
              <div class="flex justify-end gap-1">
                ...
                >Clone</button>
                <button
                  v-if="isDiscoverable(resource) && !resource.autoGenerated"
                  @click="openDiscover(resource)"
                  class="text-xs px-2 py-0.5 rounded bg-purple-900 hover:bg-purple-800 text-purple-200"
    ...
    <!-- Discover Modal -->
    <DiscoverModal
      v-if="discoverResource"
      :resourceId="discoverResource.id"
      :resourceName="discoverResource.name"
      :fetcherCode="discoverResource.fetcher?.code"
      @close="discoverResource = null"
      @created="onDiscoverCreated"
    />
...
// Discover
const discoverResource = ref(null)
function isDiscoverable(r) {
  return r.fetcher?.code?.toLowerCase().includes('portal') ||
         r.fetcher?.classPath?.toLowerCase().includes('document_portal')
}
function openDiscover(r) { discoverResource.value = r }
function onDiscoverCreated() { loadResources() }
