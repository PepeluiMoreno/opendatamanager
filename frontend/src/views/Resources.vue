  ...
  <div v-if="showImportModal"
    ...
    <div v-if="showCollectionModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="closeCollectionModal">
      <div class="bg-gray-800 rounded-xl p-6 w-full max-w-md border border-gray-700">
        <h2 class="text-2xl font-bold mb-4">{{ editingCollection ? 'Edit collection' : 'Create collection' }}</h2>
        <form @submit.prevent="submitCollection" class="space-y-4">
          <div>
            <label class="block text-sm text-gray-400 mb-1">Name</label>
            <input v-model="collectionName" type="text" autofocus
                   class="input w-full" placeholder="e.g. Entidades religiosas" />
          </div>
          <div class="flex justify-end space-x-2 pt-2">
            <button type="button" @click="closeCollectionModal" class="btn btn-secondary">Cancel</button>
            <button type="submit" :disabled="!collectionName.trim() || collectionBusy"
...
const hasActiveFilters = computed(() =>
  !!(searchQuery.value || selectedType.value || selectedPublisher.value || selectedApp.value || selectedKind.value || selectedNivel.value || !filterAllStatuses.value)
)
...
function openRenameCollection(g) { editingCollection.value = g; collectionName.value = g.name; showCollectionModal.value = true }
function closeCollectionModal() { showCollectionModal.value = false }
async function submitCollection() {
  const nombre = (collectionName.value || '').trim()
  if (!nombre) return
...
async function loadData() {
  try {
    ...
    const [rd, td, pd, rmd, pmd, cd, id, gd] = await Promise.all([
...
function origenBadge(r) {
  // Quien genero el recurso (eje humano). El antiguo origin (manifest/seed) era
  // el 'como' y se ha quedado obsoleto como etiqueta de cara al operador.
  const k = r.createdByKind || (r.autoGenerated ? 'sistema' : (['manifest','seed'].includes(r.origin) ? 'aplicacion' : 'usuario'))
  if (k === 'aplicacion') return {
    texto: 'aplicacion', clase: 'bg-blue-900 text-blue-300 border border-blue-700',
    tooltip: 'Generado por una aplicacion (via API/manifiesto), no a mano.' }
  if (k === 'sistema') return {
    texto: 'sistema', clase: 'bg-purple-900 text-purple-300 border border-purple-700',
    tooltip: 'Derivado automaticamente (crawler/descubrimiento).' }
  return {
    texto: 'usuario', clase: 'bg-gray-700 text-gray-300 border border-gray-600',
    tooltip: 'Creado a mano por un usuario en la interfaz.' }
}
