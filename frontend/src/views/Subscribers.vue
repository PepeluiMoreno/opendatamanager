  ...
  <div class="p-6 flex flex-col gap-6 h-full">
    ...
    <div class="flex flex-col min-h-0">
      <FilterBar :canClear="!!q || estadoFiltro !== 'todos'" :count="filteredApps.length" :total="subscribers.length"
        ...
        <input v-model="q" type="text" placeholder="Buscar suscriptor…" class="input text-sm w-64 max-w-[40%]" />
          ...
          <tbody>
            <tr v-for="app in filteredApps" :key="app.id" @click="selectedAppId = app.id"
              ...
              <td class="px-4 py-3 text-right whitespace-nowrap" @click.stop>
                <button @click="toggleActive(app)" :title="app.active ? 'Desactivar' : 'Activar'" class="act-icon" :class="app.active ? 'hover:text-amber-300 hover:bg-amber-900/20' : 'hover:text-emerald-300 hover:bg-emerald-900/20'">
                ...
                <button @click="openEditApp(app)" title="Editar" class="act-icon">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                </button>
                <button @click="confirmDeleteApp(app)" title="Borrar" class="act-icon danger">
            ...
            <tbody>
              <tr v-for="sub in pagedSubs" :key="sub.id"
                ...
                <td class="py-3 px-4 text-right">
                  <button @click="confirmDeleteSub(sub)" title="Borrar suscripción" class="act-icon danger"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showAppModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="closeAppModal">
      <div class="bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h2 class="text-2xl font-bold mb-4">{{ editingApp ? 'Edit subscriber' : 'Create subscriber' }}</h2>
    ...
    <div v-if="showDeleteAppModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="showDeleteAppModal = false">
      <div class="bg-gray-800 rounded-lg p-6 w-full max-w-md">
        <h2 class="text-2xl font-bold mb-4">Delete subscriber</h2>
    ...
    <div v-if="showSubModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div class="bg-gray-800 border border-gray-600 rounded-xl p-6 w-full max-w-lg shadow-2xl">
        ...
        <p class="text-sm text-gray-400 mb-5">Subscriber: <span class="text-purple-300 font-medium">{{ selectedApp?.name }}</span></p>
        <div class="space-y-4">
          <div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm text-gray-300 mb-1">Auto-upgrade <Hint text="Hasta dónde adoptas versiones nuevas solo: «patch» = siempre la última; «minor» = sigue las menores pero frena antes de un cambio mayor (puede romper esquema); «none» = no te mueve (usa la versión fijada)." /></label>
            ...
            <div>
              <label class="block text-sm text-gray-300 mb-1">Pinned version <Hint text="A qué versión(es) te ciñes («1.2.*», «1.*» o «1.2.3»). Vacío = sigues la política de auto-upgrade sobre la última." /></label>
          ...
          <div>
            <label class="block text-sm text-gray-300 mb-1">Freshness — retention before renewal (days) <Hint text="Días que toleras el dataset sin renovar antes de pedir una re-extracción. ODM concede el plazo y ordena la renovación antes de que caduque. Vacío = sin exigencia explícita (retención por defecto del recurso)." /></label>
...
async function handleDeleteApp() {
  try {
    ...
    await load()
  } catch (e) { error.value = e.message || 'Failed to delete subscriber' }
}
