<template>
  <div class="card hover:border-blue-500 transition-all cursor-pointer" @click="$emit('select', fetcherType)">
    <div class="flex items-start justify-between mb-4">
      <div>
        <h3 class="text-xl font-bold text-blue-400">{{ fetcherType.code }}</h3>
        <div class="text-xs text-gray-500 mt-1">{{ fetcherType.id }}</div>
      </div>
