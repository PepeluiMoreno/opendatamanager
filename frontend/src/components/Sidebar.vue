<template>
  <div class="w-64 h-full bg-gray-800 border-r border-gray-700 flex flex-col">
    ...
    <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto relative transition-opacity duration-300" :class="status !== 'online' ? 'opacity-40 pointer-events-none select-none' : ''">
      ...
      <NavItem v-if="puede('aplicaciones.aprobar') || puede('recursos.aprobar')" to="/aprobaciones" :active="$route.path === '/aprobaciones'">✅ Aprobaciones</NavItem>
    ...
    <div class="p-4 border-t border-gray-700 space-y-3">
      ...
      <div v-else-if="status === 'checking'" class="flex items-center gap-2 bg-yellow-900 border border-yellow-700 text-yellow-200 rounded-lg px-3 py-2 text-xs font-medium">
      ...
      <div class="flex items-center justify-between text-xs text-gray-500">
        ...
        <span class="flex items-center gap-1.5">
          <span class="inline-flex h-2 w-2 rounded-full" :class="{ 'bg-green-400': status==='online', 'bg-red-500 animate-pulse': status==='offline', 'bg-yellow-400 animate-pulse': status==='checking' }"/>
          <span :class="{ 'text-green-400': status==='online', 'text-red-400': status==='offline', 'text-yellow-400': status==='checking' }">{{ status }}</span>
        </span>
      </div>
      <div v-if="ramTotal > 0">
        ...
        <div class="h-1.5 bg-gray-700 rounded-full overflow-hidden">
          <div class="h-full rounded-full transition-all duration-700" :class="ramBarColor" :style="{ width: (100 - ramUsedPct) + '%' }"/>
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-if="help" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="help = false">
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 w-full max-w-2xl" style="max-height:85vh;overflow-y:auto">
      ...
      <div class="text-sm text-gray-300 space-y-3 leading-relaxed">
        ...
        <p class="text-gray-200 font-medium">Gobernanza (pestaña «Aprobaciones»)</p>
...
const ramUsedPct  = computed(() => ramTotal.value > 0 ? Math.round((1 - ramAvail.value / ramTotal.value) * 100) : 0)
const ramColor    = computed(() => { const f = 100 - ramUsedPct.value; return f < 15 ? 'text-red-400' : f < 30 ? 'text-yellow-400' : 'text-green-400' })
const ramBarColor = computed(() => { const f = 100 - ramUsedPct.value; return f < 15 ? 'bg-red-500' : f < 30 ? 'bg-yellow-500' : 'bg-green-500' })

let _hf = 0
async function checkHealth() {
  try {
    const r = await fetch('/health', { signal: AbortSignal.timeout(8000) })
    if (r.ok) { _hf = 0; status.value = 'online' } else if (++_hf >= 2) status.value = 'offline'
  } catch { if (++_hf >= 2) status.value = 'offline' }
}
async function fetchSysInfo() {
  try { const r = await fetch('/api/system/info', { signal: AbortSignal.timeout(3000) }); if (r.ok) { const d = await r.json(); ramTotal.value = d.ram_total_mb ?? 0; ramAvail.value = d.ram_available_mb ?? 0 } } catch {}
}
async function fetchVersion() {
  try { const r = await fetch('/api/version', { signal: AbortSignal.timeout(3000) }); if (r.ok) { const d = await r.json(); const v = d.version ?? ''; appVersion.value = v.length > 7 ? v.slice(0, 7) : v } } catch {}
}

onMounted(() => { checkHealth(); fetchSysInfo(); fetchVersion(); timer = setInterval(() => { checkHealth(); fetchSysInfo() }, 15000) })
